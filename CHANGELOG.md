## v0.2.3-alpha.6 (2026-09-27)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-alpha.6，versionCode 396

[更改] 播放设置新增可编辑的统一倍速列表，供播放器菜单、双指调速与默认速度共用；长按临时加速改用独立的 1.0x–8.0x 滑块、常用倍速快捷点及点按数值输入，离线播放器同步生效

[修复] 删除已选默认或上次倍速时自动改用最接近的有效选项，并保留升级前已有的自定义倍速

[添加] 播放器新增紧凑控制栏偏好：竖屏覆盖层与底栏按布局策略收紧，控制栏淡入淡出走统一的动效令牌

[添加] 详情页 AI 总结与视频笔记不再内联占位，改为信息区下方一行紧凑入口，内容在底部抽屉中展示

[添加] 列表页支持底部局部搜索与紧凑顶栏，搜索胶囊在列表页可正常展开收起；搜索结果编辑关键词时保持输入法不收起

[添加] 首页窄屏与宽屏分别记忆网格列数；筛选标签行下滑收起、上滑恢复，并迁移为 PiliPlus 风格色调药丸样式；结果页显示各类型数量并居中药丸标签

[添加] 收藏、历史、稍后再看全面对齐 PiliPlus：收藏夹卡片首页与夹内批量管理、批量操作顶栏、稍后再看的一键播放悬浮按钮、历史暂停横幅、选择遮罩与双指缩放列数

[更改] 主题与排版大改版：MD3 色彩规范默认 SPEC_2025 并全面改用 HCT、补齐 AMOLED 容器色覆盖、强制 4.5:1 文本对比度、CJK 阅读排版与形状刻度对齐、MD3 按钮默认紧凑密度、Miuix 标题缩放与对话框圆角对齐上游

[添加] 新增全局字重设置（四档）、字号预设扩展到 0.85x–1.25x、导入字体缺 CJK 字形时提示、Dock 标签适配系统大字体

[添加] 播放小横条与详情页卡片改用统一交接协议，新增上滑入场软回弹动效；动态顶栏操作坞折叠状态跨重启保留

[优化] 图片预览统一各入口的打开/返回锚点，修复手势返回双重回弹与圆角形变闪烁

[优化] 启动性能：首页数据就绪后一次性上报 fully drawn、跳过重复的内置默认值 runBlocking、令牌备份移出主线程；热点列表共享日期格式化、游客接口响应缓存

[优化] 内存与后台：为无界缓存设置上限、实况图保存流式落盘、直播弹幕列表用单调序列号做 key、DLNA 后台轮询降频至 5 秒、息屏放电时消息常驻轮询降频

[修复] 未登录打开"我的收藏"崩溃（#836）、弹层侧滑预测性返回关闭（#837）、搜索输入框焦点丢失（#842）、动态详情图片布局闪动（#844）、动态底栏随滚动误隐藏（#843）

[优化] 修复通知、设置分享与图片保存路径的编译错误；baselineprofile 接入 androidx.baselineprofile 1.5.0 插件
